...
export const API_TEST_SECTIONS = [
  ...
  {
    id: 'getStudentBacklog',
    label: 'My Backlog',
    title: 'GET /groups/:groupId/backlog/me',
    group: 'Backlog',
    kind: 'get',
    method: 'GET',
    hint: 'Student only. Returns recent backlog events for the logged-in student in the group (shop purchases, rank ups, badges, etc.).',
    buildPath: (values) =>
      getGroupBacklogMePath(String(values.groupId ?? '').trim(), {
        auth: values.auth,
        take: values.take,
        skip: values.skip,
      }),
    ...
    fields: [
      { key: 'groupId', label: 'Group ID (public)', type: 'number' },
      { key: 'take', label: 'take (optional, default 50, max 100)', type: 'number' },
      { key: 'skip', label: 'skip (optional, default 0)', type: 'number' },
      { key: 'auth', label: 'auth (optional if cookie set)', type: 'textarea' },
    ],
  },
  {
    id: 'getGroupBacklog',
    label: 'Group Backlog',
    title: 'GET /groups/:groupId/backlog',
    group: 'Backlog',
    kind: 'get',
    method: 'GET',
    hint: 'Lecturer/admin only. Returns backlog events for all members in the group. Requires group ownership (teacherAccountId) unless SUPER.',
    buildPath: (values) =>
      getGroupBacklogPath(String(values.groupId ?? '').trim(), {
        auth: values.auth,
        take: values.take,
        skip: values.skip,
      }),
